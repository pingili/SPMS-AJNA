...
namespace BusinessLogic
{
    public class JournalService 
    {
        ...
        public List<JournalLookupDto> Lookup()
        {
            ...
            List<uspJournalEntryLookup_Result> lstuspGeneralPaymentsLookup_Result = _dbContext.uspJournalEntryLookup().ToList();
            foreach (var JournalEntry in lstuspGeneralPaymentsLookup_Result)
            {
                JournalLookupDto JournalEntryDto = Mapper.Map<uspJournalEntryLookup_Result, JournalLookupDto>(JournalEntry);
        ...
        public ReceiptMasterDto GetByID(long AccountMasterId)
        {
            ...
            var results = new MFISDBContext()
                .MultipleResults(ProcNames.uspJournalEntryVoucherGetById, prmAccountMasterId)
