...
{
    public class FederationGeneralReceiptService 
    {
        #region Global Variables
        private readonly MFISDBContext _dbContext;
        private readonly CommonService _commonService;

        public FederationGeneralReceiptService()
        {
        ...
        private ResultDto insertupdateGeneralreceipt(GeneralReceiptDto generalreceiptDto)
        {
            ...
            generalreceiptDto.IsGroup = Convert.ToBoolean(0);
        ...
        public List<GeneralReceiptLookupDto> Lookup()
        {
            ...
            List<uspFederationGeneralReceiptLookup_Result> lstuspGeneralPaymentsLookup_Result = _dbContext.uspFederationGeneralReceiptLookup().ToList();
          
            foreach (var GeneralReceipt in lstuspGeneralPaymentsLookup_Result)
            {
                GeneralReceiptLookupDto GeneralreceiptDto = Mapper.Map<uspFederationGeneralReceiptLookup_Result, GeneralReceiptLookupDto>(GeneralReceipt);
        ...
        public GeneralReceiptDto GetByID(long AccountMasterId)
        {
            ...
            var results = new MFISDBContext()
                .MultipleResults(ProcNames.uspGeneralReceiptGetById, prmAccountMasterId)
                .With<GeneralReceiptDto>()
                .With<AddAmountDto>()
                .Execute();

            var generalReceiptDto = (results[0] as List<GeneralReceiptDto>)[0];
